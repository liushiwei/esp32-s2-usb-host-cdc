...
esp_err_t hcd_irp_enqueue(hcd_pipe_handle_t pipe_hdl, usb_irp_t *irp)
{
    ...
    pipe_t *pipe = (pipe_t *)pipe_hdl;
    ESP_LOGE("", "hcd_irp_enqueue %p ",pipe );
    ESP_LOGE("", "hcd_irp_enqueue 111 " );
    ESP_LOGE("", "hcd_irp_enqueue 111 %d",pipe->port->state == HCD_PORT_STATE_ENABLED );
    ESP_LOGE("", "hcd_irp_enqueue 111 %d",pipe->state == HCD_PIPE_STATE_ACTIVE );
    ESP_LOGE("", "hcd_irp_enqueue 112 %d",!pipe->cs_flags.pipe_cmd_processing );
    // HCD_ENTER_CRITICAL();
